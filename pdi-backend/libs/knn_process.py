...
from libs.segmentation import segmentar_objeto_com_flood_fill, filtrar_contornos_borda, encontrar_contornos, \
    desenhar_contornos
...
def knn_process_df_image(image_path=None, image_process=None):
    ...
    return {'image_process': np.ravel(image_process), 'img_rgb': np.ravel(img_rgb), 'img_cinza': np.ravel(img_cinza),
            'img_suavizada': np.ravel(img_suavizada), 'img_bordas_canny': np.ravel(img_bordas_canny),
            'mascara_segmentada': np.ravel(mascara_segmentada), 'contornos': ensure_flatten(contornos),
            'altura_img': np.ravel(altura_img), 'largura_img': np.ravel(largura_img),
            'contornos_filtrados': ensure_flatten(contornos_filtrados), 'mascara_final': np.ravel(mascara_final),
            'img_com_contornos': np.ravel(img_com_contornos), 'metricas_geo': np.ravel(metricas_geo),
            'vetor_hog': np.ravel(vetor_hog), 'img_visual_hog': np.ravel(img_visual_hog),
            'img_visual_lbp': np.ravel(img_visual_lbp), 'hist_lbp': np.ravel(hist_lbp),
